...
RUN echo '1' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/1.sh

RUN echo 'cd ' >>/1.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/1.sh

RUN echo 'cd /noVNC-1.2.0' >>/1.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/1.sh
